    ...
    if(ifirst == 0) { // Initialization
        ...
        for(i=0; i<=5; i++) {
            ...
            for (ib=1; ib<=nBdata; ib++) {
                ...
                istringstream s(str);
                s >> chatmp;
                for(ia=1;ia<=nAdata;ia++) {s >> A[i][ib][ia];} // A(4)&A(12) is s,r,d-dependence, so will be changed
            }
        }
        ...
        for(i=npart-5;i<=npart;i++) {
            ...
            for(ib=1;ib<=nBdata;ib++) {
                ...
                istringstream s1(str);
                s1 >> chatmp;
                for(ia=1;ia<=nAdata;ia++) {s1 >> A[i][ib][ia];} // A(4)&A(12) is s,r,d-dependence, so will be changed
            }
        }
    }
...
// *******************************************************
{
    ...
    if(ifirst ==0) { // Initialization
        ifirst = 1;
        for(i=1;i<=5;i++) {
            if(i==1) {dname="input/proton/fitting-lowspec.inp";}
            else if(i==2) {dname="input/alphaa/fitting-lowspec.inp";}
            else if(i==3) {dname="input/elemag/fitting-lowspec-EL.inp";}
            else if(i==4) {dname="input/elemag/fitting-lowspec-PO.inp";}
            else if(i==5) {dname="input/elemag/fitting-lowspec-PH.inp";}
            ifstream ifs(dname,ios::in);
            getline(ifs, str); // Kara-yomi
            for(id=1;id<=ndep;id++) {
                getline(ifs, str);
...
// *******************************************************
{
    ...
    if(ifirst == 0) { // Initialization
        ...
        for(i=1;i<=ngroup;i++) {
            getline(ifs, str);
            istringstream s(str);
            for(ia=1;ia<=nAdata;ia++) {s >> a[ia][i];} // A(4)&A(12) is s,r,d-dependence, so will be changed
        }
    }
...
// *******************************************************
{
    ...
    if(i==7) {getParaAdep=max(0.1,min(1.0,getParaAdep));} // a(7) should be 0.1 < a7 1.0
    if(i==1 || i==4 || i==8) {getParaAdep=max(0.0,getParaAdep);} // a(1), a(4) & a(8) should be positive
...
//     ParaAdep[i][idx]=getParaAdepCpp(i,ene,ParaEdep[1][i][ic1][is1][ip]);
                        ...
                        if(ip==1 && g>=0.0 && g<=1.0) ParaAdep[i][idx]=ParaAdep[i][idx]+getGneutCpp(e,i); // Ground level neutron correction, e instead of ene is used because data are down to 1e-8
                        if(i==2 && (ParaAdep[1][idx]+ParaAdep[2][idx])<phimin) ParaAdep[2][idx]=-ParaAdep[1][idx]+min(ParaAdep[1][idx],phimin);  // avoid negative value at cos(theta)=-1.0
                        if(i==5 && (ParaAdep[4][idx]+ParaAdep[5][idx])<phimin) ParaAdep[5][idx]=-ParaAdep[4][idx]+min(ParaAdep[4][idx],phimin);  // avoid negative value at cos(theta)=1.0
                    }
                }
            }
        }
...
//  **** No data **************************
    ic=4;
    getHP=0.0;
    if(ic==4) {cout << "No FFP data is available" << "\n";}
    return getHP;
}
...
PARMAEvent PARMAGenerator::generate() {
